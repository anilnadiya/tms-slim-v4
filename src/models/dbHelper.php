...
class dbHelper
{
    ...
    //     // Make sure the environment variables are not empty
    //     if (empty($host) || empty($database)) {
    //         throw new \Exception('Database credentials are missing or incorrect in the environment variables.');
    //     }
    ...
    public function update($tableName, $tableData) {
        if ($this->isSubQuery)
            return;
        $tableData = array_map('trim',$tableData);
    ...
    protected function _dynamicBindResults(\mysqli_stmt $stmt) {
        ...
        // If $meta is false yet sqlstate is true, there's no SQL error
        // The query is most likely an update/insert/delete which doesn't produce any results
